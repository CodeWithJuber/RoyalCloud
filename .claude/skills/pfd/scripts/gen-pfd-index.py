...
# Support both layouts: original plugin repo (skills/pfd/references) and a
# flattened install where everything lives in the skill directory itself.
_CANDIDATES = [
    SCRIPT_DIR.parent / "skills" / "pfd" / "references" / "learnings",
    SCRIPT_DIR.parent / "references" / "learnings",
]
LEARNINGS_ROOT = next((c for c in _CANDIDATES if c.exists()), _CANDIDATES[0])
