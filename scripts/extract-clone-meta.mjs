...
function extract(html) {
  ...
  return {
    ...
    description: pick(/<meta\s+name=["']description["']\s+content=["']([\s\S]*?)["']/i),
    keywords: pick(/<meta\s+name=["']keywords["']\s+content=["']([\s\S]*?)["']/i),
...
function landingDoc(slug, meta) {
  ...
  const front = {
    ...
    sections: [
      ...
      { type: 'techlogos', eyebrow: 'Powered By', title: 'Technology partners we build on' },
      { type: 'testimonials', eyebrow: 'Reviews', title: 'Loved by website owners', source: 'global', limit: 3 },
      { type: 'faq', eyebrow: 'FAQ', title: `${kw} — common questions`, source: 'inline', items: fam.faq(kw, distro) },
      {
        type: 'cta',
...
for (const slug of slugs) {
  ...
  const meta = all[slug] || { title: '', description: '', keywords: '', h1: '' };
  writeFileSync(out, target === 'landing' ? landingDoc(slug, meta) : pageDoc(slug, meta));
  written++;
}
